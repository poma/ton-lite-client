...
struct IhrPendingSince final : TLB_Complex {
  ...
  bool unpack(vm::CellSlice& cs, Record& data) const;
  bool unpack_ihr_pending(vm::CellSlice& cs, unsigned long long& shard, unsigned long long& import_lt) const;
  bool cell_unpack(Ref<vm::Cell> cell_ref, Record& data) const;
  bool cell_unpack_ihr_pending(Ref<vm::Cell> cell_ref, unsigned long long& shard, unsigned long long& import_lt) const;
  bool pack(vm::CellBuilder& cb, const Record& data) const;
  bool pack_ihr_pending(vm::CellBuilder& cb, unsigned long long shard, unsigned long long import_lt) const;
  bool cell_pack(Ref<vm::Cell>& cell_ref, const Record& data) const;
  bool cell_pack_ihr_pending(Ref<vm::Cell>& cell_ref, unsigned long long shard, unsigned long long import_lt) const;
  bool print_skip(PrettyPrinter& pp, vm::CellSlice& cs) const override;
  std::ostream& print_type(std::ostream& os) const override {
...
struct ShardStateUnsplit_aux::Record {
  ...
  Record() = default;
  Record(unsigned long long _overload_history, unsigned long long _underload_history, Ref<CellSlice> _total_balance, Ref<CellSlice> _total_validator_fees, Ref<CellSlice> _libraries) : overload_history(_overload_history), underload_history(_underload_history), total_balance(std::move(_total_balance)), total_validator_fees(std::move(_total_validator_fees)), libraries(std::move(_libraries)) {}
};
...
struct ShardStateUnsplit::Record {
  ...
  ShardStateUnsplit_aux::Record r1;  	// ^[$_ overload_history:uint64 underload_history:uint64 total_balance:CurrencyCollection total_validator_fees:CurrencyCollection libraries:(HashmapE 256 LibDescr) ]
  Ref<CellSlice> master_ref;  	// master_ref : Maybe BlkMasterInfo
  Ref<CellSlice> custom;  	// custom : Maybe ^McStateExtra
  Record() = default;
  Record(int _global_id, Ref<CellSlice> _shard_id, unsigned _seq_no, int _vert_seq_no, unsigned _gen_utime, unsigned long long _gen_lt, unsigned _min_ref_mc_seqno, Ref<Cell> _out_msg_queue_info, bool _before_split, Ref<CellSlice> _accounts, const ShardStateUnsplit_aux::Record& _r1, Ref<CellSlice> _master_ref, Ref<CellSlice> _custom) : global_id(_global_id), shard_id(std::move(_shard_id)), seq_no(_seq_no), vert_seq_no(_vert_seq_no), gen_utime(_gen_utime), gen_lt(_gen_lt), min_ref_mc_seqno(_min_ref_mc_seqno), out_msg_queue_info(std::move(_out_msg_queue_info)), before_split(_before_split), accounts(std::move(_accounts)), r1(_r1), master_ref(std::move(_master_ref)), custom(std::move(_custom)) {}
};
...
// ^[$_ overload_history:uint64 underload_history:uint64 total_balance:CurrencyCollection total_validator_fees:CurrencyCollection libraries:(HashmapE 256 LibDescr) ]

...
bool IhrPendingSince::unpack_ihr_pending(vm::CellSlice& cs, unsigned long long& shard, unsigned long long& import_lt) const {
  return cs.fetch_uint_to(64, shard)
      && cs.fetch_uint_to(64, import_lt);
}
...
bool IhrPendingSince::cell_unpack_ihr_pending(Ref<vm::Cell> cell_ref, unsigned long long& shard, unsigned long long& import_lt) const {
  if (cell_ref.is_null()) { return false; }
  auto cs = load_cell_slice(std::move(cell_ref));
  return unpack_ihr_pending(cs, shard, import_lt) && cs.empty_ext();
}
...
bool IhrPendingSince::pack_ihr_pending(vm::CellBuilder& cb, unsigned long long shard, unsigned long long import_lt) const {
  return cb.store_ulong_rchk_bool(shard, 64)
      && cb.store_ulong_rchk_bool(import_lt, 64);
}
...
bool IhrPendingSince::cell_pack_ihr_pending(Ref<vm::Cell>& cell_ref, unsigned long long shard, unsigned long long import_lt) const {
  vm::CellBuilder cb;
  return pack_ihr_pending(cb, shard, import_lt) && std::move(cb).finalize_to(cell_ref);
}
